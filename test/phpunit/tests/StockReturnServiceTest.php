...
class StockReturnServiceTest extends CommonClassTest
{
	...
	private function createProduct($suffix, $type = Product::TYPE_PRODUCT, $stockable = 1)
	{
		global $conf;

		$ref = 'DSR-'.$suffix.'-'.mt_rand(10000, 99999);
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."product";
		$sql .= " (ref, label, fk_product_type, entity, tosell, tobuy, stockable_product, duration, datec, tms)";
		$sql .= " VALUES ('".$this->db->escape($ref)."', '".$this->db->escape($ref)."', ".((int) $type).", ".((int) $conf->entity).", 1, 1, ".((int) $stockable).", '', '".$this->db->idate(dol_now())."', '".$this->db->idate(dol_now())."')";
	...
	private function insertStockMovement($originId, $originType, $productId, $warehouseId, $value)
	{
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."stock_mouvement";
		$sql .= " (datem, fk_product, fk_entrepot, value, price, type_mouvement, fk_user_author, label, inventorycode, origintype, fk_origin)";
		$sql .= " VALUES ('".$this->db->idate(dol_now())."', ".((int) $productId).", ".((int) $warehouseId).", ".((float) $value).", 10, ";
	...
	private function assertReturnDetail($returnId, $productId, $warehouseId, $qty)
	{
